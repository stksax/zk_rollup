...
$$private key * g (generater) = public key  commitment= random number * g  challenge=hash (sender's private key, reciver's public key, payment) response=random number + challenge*sender's private key (it's a Diffie–Hellman key exchange) $$
