...
describe("after verify the transactions, use the result as new merkle tree leaf, and rollup a new root", function () {
    ...
    before( async () => {
        ...
        update_leaf1= await wasm_tester(path.join(__dirname, "circuits", "update_leaf1.circom"));
        update_leaf2= await wasm_tester(path.join(__dirname, "circuits", "update_leaf2.circom"));
        rollup= await wasm_tester(path.join(__dirname, "circuits", "rollup.circom"));
        generate_leaf= await wasm_tester(path.join(__dirname, "circuits", "generate_leaf.circom"));
    });
